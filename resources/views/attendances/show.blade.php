            ...
            <div class="overflow-hidden rounded-lg bg-white p-4 shadow-sm sm:p-6">
                ...
                <form action="{{ route('attendance-correction-requests.store', $attendance) }}" method="POST">
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <button id="add-break" type="button" class="touch-target w-full rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white hover:bg-green-700 sm:w-auto">休憩を追加</button>
                    </div>
                        ...
                        @foreach ($breakRows as $index => $break)
                            <div class="break-row rounded-lg bg-gray-100 p-4">
                                <div class="grid gap-4 sm:grid-cols-[1fr_1fr_auto]">
                                    ...
                                    <button type="button" class="remove-break touch-target w-full rounded-lg bg-red-50 px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-100 sm:mt-8 sm:w-auto">削除</button>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="mt-8 flex flex-col gap-3 sm:flex-row">
                        <button
                            type="submit"
                            class="touch-target w-full rounded-md bg-blue-600 px-5 py-2 text-white hover:bg-blue-700 sm:w-auto">
                            修正申請を送信
                        </button>

                        <a
                            href="{{ route('attendances.index', ['month' => $attendance->work_date->format('Y-m')]) }}"
                            class="touch-target w-full rounded-md bg-gray-600 px-5 py-2 text-center text-white hover:bg-gray-700 sm:w-auto">
                            一覧へ戻る
                        </a>
                    </div>
