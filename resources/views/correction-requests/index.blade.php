    ...
    <div class="py-8">
        <div class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
            <div class="flex rounded-xl border border-gray-100 bg-white p-2 shadow-sm">
                <a href="{{ route('correction-requests.index', ['status' => 'pending']) }}" class="touch-target flex-1 rounded-lg px-3 py-2 text-center text-sm font-semibold {{ $status === 'pending' ? 'bg-yellow-500 text-white' : 'text-gray-600 hover:bg-gray-50' }}">承認待ち</a>
                <a href="{{ route('correction-requests.index', ['status' => 'approved']) }}" class="touch-target flex-1 rounded-lg px-3 py-2 text-center text-sm font-semibold {{ $status === 'approved' ? 'bg-green-600 text-white' : 'text-gray-600 hover:bg-gray-50' }}">承認済み</a>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($correctionRequests as $correctionRequest)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-right"><a href="{{ route('correction-requests.show', $correctionRequest) }}" class="touch-target rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">詳細</a></td>
                                </tr>
                            @empty
